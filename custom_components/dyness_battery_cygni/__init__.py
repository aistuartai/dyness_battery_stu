...
class DynessDataCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass, api_id, api_secret, api_base,
        ...
        self._last_alarm_texts: list = []        # track alarm state to avoid repeated notifications
            ...
            try:
                async with session.post(url, headers=headers, data=body) as response:
                    ...
                    try:
                        ...
                        _LOGGER.warning("Dyness %s: non-JSON response (HTTP %s): %s",
                                        path, response.status, raw_text[:200])
                        return {}
            ...
            try:
                async with session.post(url, headers=headers, data=body) as response:
                    ...
                    try:
                        ...
                        _LOGGER.warning("Dyness v2 %s: non-JSON response (HTTP %s): %s",
                                        path, response.status, raw_text[:200])
                        return {}
            except aiohttp.ClientError as e:
        ...
        try:
            async with async_timeout.timeout(90):
                ...
                if alarm_texts:
                    ...
                    if alarm_texts != self._last_alarm_texts:
                        self._last_alarm_texts = alarm_texts
                    ...
                    data["alarmText"] = "OK"
                    if self._last_alarm_texts:
                        self._last_alarm_texts = []
                        self.hass.async_create_task(
                            self.hass.services.async_call(
